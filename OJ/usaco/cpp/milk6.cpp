...
const long long infinity = (long long)10000000 * 10000000;
struct edgenode
{
	int a, b;
} edge[maxm + 1];
int n, m;
long long a[maxn + 1][maxn + 1], aug[maxn + 1];
...
void init()
{
	...
	for (int i = 1; i <= m; ++ i)
	{
		...
		a[edge[i].a][edge[i].b] += (long long)(k * 1001 + 1) * 500000 + (i - 1);
	}
}
...
void cut()
{
	fout << ans / 500500000 << " " << (ans / 500000) % 1001 << endl;
