...
procedure maintain(var t:tree);
 begin
     if t<>nullnode then t^.size:=t^.left^.size+t^.right^.size+1;
 end;
...
procedure delete(var x:longint; var t:tree);
 begin
	 ...
	 if (t=last) and (deleted<>nullnode) and (x=deleted^.key) then begin
	     ...
	     maintain(t);
	     if (t^.left^.level<t^.level-1) or (t^.right^.level<t^.level-1) then begin
	         dec(t^.level);
...
function rank(var t:tree; x:longint):longint;
 begin
     if t=nullnode then exit(1);
     if x<=t^.key then rank:=rank(t^.left,x)
	else rank:=t^.left^.size+1+rank(t^.right,x);
 end;

function select(var t:tree; k:longint):longint;
 begin
     if k=t^.left^.size+1 then exit(t^.key);
     if k<=t^.left^.size then select:=select(t^.left,k)
	else select:=select(t^.right,k-1-t^.left^.size);
 end;
