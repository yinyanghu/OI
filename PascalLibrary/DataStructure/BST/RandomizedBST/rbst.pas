  ...
  begin
      if t=0 then begin
         ...
         s[t]:=1;
          ...
          if not pd and (random(s[t]+1)=0) then pd:=true;
          inc(s[t]);
          if v<key[t] then begin
              randomizedinsert(left[t],v,pd);
              if pd then right_rotate(t);
           end else begin
              randomizedinsert(right[t],v,pd);
              if pd then left_rotate(t);
          end;
      end;
  end;
...
begin
    assign(input,'rbst.in'); reset(input);
    assign(output,'rbst.out'); rewrite(output);
    init;
    work;
    close(input);
    close(output);
